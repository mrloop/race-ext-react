...
export default class SetupEvents {
  ...
  initDesktop() {
    Array.from(
      this.table.getElementsByClassName("events--desktop__row")
    ).forEach((row, index) => {
      if (index === 0) {
        this.addHeader(row);
      } else {
  ...
  initMobile() {
    Array.from(
      this.table.getElementsByClassName("events--mobile__row")
    ).forEach((row, index) => {
      if (index > 0) {
        let link = row.querySelector("[data-event-id]");
        if (link) {
          let id = link.getAttribute("data-event-id");
          let buttons = row.getElementsByClassName(
  ...
  tidyUp() {
    ["race-ext__mobile", "race-ext__desktop", "race-ext-modal"].forEach(
      (className) => {
        Array.from(this.doc.getElementsByClassName(className)).forEach((elem) =>
          elem.remove()
        );
      }
    );
  ...
  render(component, element) {
    ReactDOM.render(<React.StrictMode>{component}</React.StrictMode>, element);
  }
}
